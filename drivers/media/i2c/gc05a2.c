			     ...
			     struct v4l2_subdev_format *fmt)
{
	...
	mbus_fmt = v4l2_subdev_get_pad_format(sd, sd_state, fmt->pad);
	*mbus_fmt = fmt->format;
...
static int gc05a2_set_ctrl(struct v4l2_ctrl *ctrl)
{
	...
	format = v4l2_subdev_get_pad_format(&gc05a2->sd, state, 0);
	 ...
	 */
	if (!pm_runtime_get_if_active(gc05a2->dev, true))
		return 0;
